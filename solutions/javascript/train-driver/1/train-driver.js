...
export function fixListOfWagons(ids) {
  let arrArgs = [...ids];
  const firstWagons = arrArgs.slice(0, arrArgs.indexOf(1));
  arrArgs.splice(0, firstWagons.length);

  return  [...arrArgs, ...firstWagons];
}
...
export function correctListOfWagons(ids, missingWagons) {
  let fixedListOfWagons =  fixListOfWagons(ids);
  fixedListOfWagons.splice(1, 0, ...missingWagons);
...
export function separateTimeOfArrival(information) {
  let timeOfArrival = information.timeOfArrival;
  let copiedInfo = Object.assign({}, information);
  delete copiedInfo.timeOfArrival;

  return [timeOfArrival, copiedInfo];
}
